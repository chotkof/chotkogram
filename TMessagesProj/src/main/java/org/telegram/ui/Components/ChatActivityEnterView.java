...
//        }
    ...
    public boolean processSendingText(CharSequence text, boolean notify, int scheduleDate, int scheduleRepeatPeriod, long payStars) {
        ...
        if (!AyuConfig.sendFunCommands && AyuFunController.isFunCommand(text)) {
            if (parentFragment != null) {
                BulletinFactory.of(parentFragment).createSimpleBulletin(R.raw.info, "Sending troll commands is disabled").show();
            }
            return false;
        }
        ...
        if (text.length() != 0) {
            ...
            do {
                ...
                SendMessagesHelper.getInstance(currentAccount).sendMessage(params);
                if (scheduleDate == 0) {
                    AyuFunController.playOutgoing(message[0]);
                }
                start = end + 1;
