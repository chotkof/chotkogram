...
//                }
                ...
                ArrayList<Long> ayuSavedDialogs = new ArrayList<>();
                ...
                try {
                    while (cursor.next()) {
                        ...
                        if (data != null) {
                            ...
                            int topicId = (int) MessageObject.getTopicId(currentAccount, message, getForumTypeFlags(did));
                            var prefs = new AyuSavePreferences(message, currentAccount, did, topicId, mid, getConnectionsManager().getCurrentTime());
                            AyuMessagesController.rememberMessage(prefs);
                            AyuMessagesController.getInstance().onMessageDeleted(prefs);
                            if (!ayuSavedDialogs.contains(did)) {
                                ayuSavedDialogs.add(did);
                            }
                ...
                try {
                    while (cursor.next()) {
                        ...
                        if (data != null) {
                            ...
                            var prefs = new AyuSavePreferences(message, currentAccount, did, (int) topicId, mid, getConnectionsManager().getCurrentTime());
                            AyuMessagesController.rememberMessage(prefs);
                            AyuMessagesController.getInstance().onMessageDeleted(prefs);
                            if (!ayuSavedDialogs.contains(did)) {
                                ayuSavedDialogs.add(did);
                ...
                ArrayList<Integer> missingAyuMessages = new ArrayList<>(unknownMessages);
                missingAyuMessages.retainAll(unknownMessagesInTopics);
                Map<Long, ArrayList<Integer>> cachedSavedMessages = AyuMessagesController.getInstance().saveCachedDeletedMessages(currentAccount, dialogId, missingAyuMessages, getConnectionsManager().getCurrentTime());
                for (Map.Entry<Long, ArrayList<Integer>> entry : cachedSavedMessages.entrySet()) {
                    ...
                    ArrayList<Integer> savedIds = entry.getValue();
                    for (int j = 0; j < savedIds.size(); j++) {
                        Integer mid = savedIds.get(j);
                        if (!mids.contains(mid)) {
                            mids.add(mid);
                        }
                    }
                }
                if (!ayuSavedDialogs.isEmpty()) {
                    for (int i = 0; i < ayuSavedDialogs.size(); i++) {
                        long did = ayuSavedDialogs.get(i);
                        ArrayList<Integer> mids = messagesByDialogs.get(did);
                        AndroidUtilities.runOnUIThread(() -> getNotificationCenter().postNotificationName(com.radolyn.ayugram.AyuConstants.MESSAGES_DELETED_NOTIFICATION, did, mids));
                    }
                }
    ...
    private ArrayList<Long> markMessagesAsDeletedInternal(long channelId, int mid, boolean deleteFiles) {
        ...
        try {
            ...
            try {
                while (cursor.next()) {
                    ...
                    if (data != null) {
                        ...
                        int topicId = (int) MessageObject.getTopicId(currentAccount, message, getForumTypeFlags(did));
                        var prefs = new AyuSavePreferences(message, currentAccount, did, topicId, messageId, getConnectionsManager().getCurrentTime());
                        AyuMessagesController.rememberMessage(prefs);
                        AyuMessagesController.getInstance().onMessageDeleted(prefs);
                        if (!ayuSavedDialogs.contains(did)) {
                            ayuSavedDialogs.add(did);
            ...
            if (!ayuSavedDialogs.isEmpty()) {
                for (int i = 0; i < ayuSavedDialogs.size(); i++) {
                    long did = ayuSavedDialogs.get(i);
                    AndroidUtilities.runOnUIThread(() -> getNotificationCenter().postNotificationName(com.radolyn.ayugram.AyuConstants.MESSAGES_DELETED_NOTIFICATION, did, null));
                }
            }
